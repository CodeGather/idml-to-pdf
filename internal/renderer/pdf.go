...
func (r *PDFRenderer) loadFont(name, path string) {
	if _, ok := r.fontLoaded[name]; ok { return }
	if _, err := os.Stat(path); err == nil {
		if err := r.pdf.AddTTFFont(name, path); err == nil { r.fontLoaded[name] = true }
	}
}
...
func (r *PDFRenderer) mapFont(idmlFont string) string {
	...
	if v, ok := m[idmlFont]; ok { return v }
	lower := strings.ToLower(idmlFont)
	if strings.Contains(lower, "hei") { return "SimHei" }
	if strings.Contains(lower, "song") || strings.Contains(lower, "ming") { return "ArialUnicode" }
	if strings.Contains(lower, "yahei") || strings.Contains(lower, "microsoft") { return "微软雅黑" }
	return "ArialUnicode"
}
...
func (r *PDFRenderer) DrawImage(imageData []byte, x, y, w, h, angle float64) error {
	if len(imageData) == 0 { return fmt.Errorf("empty image data") }
	tmpDir := os.TempDir()
	ext := ".png"
	if len(imageData) > 2 && imageData[0] == 0xFF && imageData[1] == 0xD8 { ext = ".jpg" } else if len(imageData) > 4 && string(imageData[:4]) == "%PDF" { ext = ".pdf" }
	tmpFile := filepath.Join(tmpDir, fmt.Sprintf("idmlimg_%d%s", len(imageData), ext))
	if err := os.WriteFile(tmpFile, imageData, 0644); err != nil { return fmt.Errorf("write temp image: %w", err) }
	defer os.Remove(tmpFile)
	drawFile := tmpFile
	if ext == ".pdf" {
		pngFile := tmpFile + ".png"
		if err := convertPDFToPng(tmpFile, pngFile); err != nil { return fmt.Errorf("convert pdf/ai to png: %w", err) }
		defer os.Remove(pngFile)
		drawFile = pngFile
		opaqueFile := tmpFile + ".opaque.png"
		if err := ensureOpaqueWhiteBackground(drawFile, opaqueFile); err == nil { defer os.Remove(opaqueFile); drawFile = opaqueFile }
		_, _, cropErr := cropWhiteBackground(drawFile, drawFile+".cropped.png")
		if cropErr == nil { defer os.Remove(drawFile + ".cropped.png"); drawFile = drawFile + ".cropped.png" }
	}
...
func (r *PDFRenderer) DrawRotatedImage(imageData []byte, cx, cy, w, h, angle float64) error {
	...
	if ext == ".pdf" {
		pngFile := tmpFile + ".png"
		if err := convertPDFToPng(tmpFile, pngFile); err != nil { return fmt.Errorf("convert pdf/ai to png: %w", err) }
		defer os.Remove(pngFile); drawFile = pngFile
		opaqueFile := tmpFile + ".opaque.png"
		if err := ensureOpaqueWhiteBackground(drawFile, opaqueFile); err == nil { defer os.Remove(opaqueFile); drawFile = opaqueFile }
		_, _, cropErr := cropWhiteBackground(drawFile, drawFile+".cropped.png")
		if cropErr == nil { defer os.Remove(drawFile + ".cropped.png"); drawFile = drawFile + ".cropped.png" }
...
func CropImageToVisibleRegion(imgData []byte, pathLx1, pathLy1, pathLx2, pathLy2 float64, imgTx, imgTy, imgSx, imgSy float64, gbRight, gbBottom float64) []byte {
	if imgSx == 0 || imgSy == 0 || gbRight == 0 || gbBottom == 0 { return imgData }
	srcX1 := (pathLx1 - imgTx) / imgSx; srcY1 := (pathLy1 - imgTy) / imgSy
	srcX2 := (pathLx2 - imgTx) / imgSx; srcY2 := (pathLy2 - imgTy) / imgSy
	if srcX1 > srcX2 { srcX1, srcX2 = srcX2, srcX1 }
	if srcY1 > srcY2 { srcY1, srcY2 = srcY2, srcY1 }
	if srcX2-srcX1 >= gbRight*0.95 && srcY2-srcY1 >= gbBottom*0.95 { return imgData }
	if srcX1 < 0 { srcX1 = 0 }; if srcY1 < 0 { srcY1 = 0 }
	if srcX2 > gbRight { srcX2 = gbRight }; if srcY2 > gbBottom { srcY2 = gbBottom }
	if srcX1 >= srcX2 || srcY1 >= srcY2 { return imgData }
	meta, err := bimg.NewImage(imgData).Metadata()
	if err != nil || meta.Size.Width == 0 || meta.Size.Height == 0 { return imgData }
	pixW, pixH := meta.Size.Width, meta.Size.Height
	pX1 := int(srcX1 * float64(pixW) / gbRight); pY1 := int(srcY1 * float64(pixH) / gbBottom)
	pX2 := int(srcX2 * float64(pixW) / gbRight); pY2 := int(srcY2 * float64(pixH) / gbBottom)
	if pX1 < 0 { pX1 = 0 }; if pY1 < 0 { pY1 = 0 }; if pX2 > pixW { pX2 = pixW }; if pY2 > pixH { pY2 = pixH }
	if pX1 >= pX2 || pY1 >= pY2 { return imgData }
	opts := bimg.Options{AreaWidth: pX2 - pX1, AreaHeight: pY2 - pY1, Top: pY1, Left: pX1, Quality: 90}
	if meta.Space == "cmyk" || meta.Space == "CMYK" { opts.Interpretation = bimg.InterpretationCMYK }
	cropped, err := bimg.NewImage(imgData).Process(opts)
	if err != nil { return imgData }
	return cropped
}
...
func convertPDFToPng(src, dst string) error {
	...
	if filepath.Ext(strings.ToLower(src)) == ".ai" {
		...
		if data, err := os.ReadFile(src); err == nil {
			os.WriteFile(tmpPDF, data, 0644); defer os.Remove(tmpPDF); pdfSrc = tmpPDF
		}
	}
	const rasterDPI = 600
	if err := exec.Command("mutool", "convert", "-O", "resolution="+strconv.Itoa(rasterDPI), "-o", dst, pdfSrc, "1").Run(); err == nil {
		...
		if _, err := os.Stat(generated); err == nil { os.Rename(generated, dst); return nil }
	}
	outPrefix := dst + "_ppm"
	if err := exec.Command("pdftoppm", "-png", "-f", "1", "-l", "1", "-r", strconv.Itoa(rasterDPI), pdfSrc, outPrefix).Run(); err == nil {
		generated := outPrefix + "-1.png"
		if _, err := os.Stat(generated); err == nil { os.Rename(generated, dst); os.Remove(outPrefix + "-1.png"); return nil }
	}
	switch runtime.GOOS {
	case "darwin":
		if err := exec.Command("sips", "-s", "format", "png", src, "--out", dst).Run(); err == nil { return nil }
	case "windows":
		if err := exec.Command("magick", "convert", "-density", strconv.Itoa(rasterDPI), pdfSrc+"[0]", dst).Run(); err == nil { return nil }
		if err := exec.Command("convert", "-density", strconv.Itoa(rasterDPI), pdfSrc+"[0]", dst).Run(); err == nil { return nil }
		if err := exec.Command("gswin64c", "-sDEVICE=pngalpha", "-dFirstPage=1", "-dLastPage=1", "-r"+strconv.Itoa(rasterDPI), "-o", dst, pdfSrc).Run(); err == nil { return nil }
		if err := exec.Command("gswin32c", "-sDEVICE=pngalpha", "-dFirstPage=1", "-dLastPage=1", "-r"+strconv.Itoa(rasterDPI), "-o", dst, pdfSrc).Run(); err == nil { return nil }
	default:
		if err := exec.Command("convert", "-density", strconv.Itoa(rasterDPI), pdfSrc+"[0]", dst).Run(); err == nil { return nil }
		if err := exec.Command("gm", "convert", "-density", strconv.Itoa(rasterDPI), pdfSrc+"[0]", dst).Run(); err == nil { return nil }
	}
...
func ensureOpaqueWhiteBackground(src, dst string) error {
	...
	for y := bounds.Min.Y; y < bounds.Max.Y; y++ {
		for x := bounds.Min.X; x < bounds.Max.X; x++ { white.Set(x, y, image.White) }
	}
...
func (r *PDFRenderer) parseColorCMYK(idmlColor string) (c, m, y, k uint8) {
	switch idmlColor {
	case "Color/Black": return 0, 0, 0, 100
	case "Color/None", "Swatch/None": return 0, 0, 0, 0
	}
...
func (r *PDFRenderer) DrawTextFrame(x, y, w, h float64, text string, fontName string, fontSize float64, angle float64, hAlign string, textColor string) {
	...
	if hasCJK && ((ang > 85 && ang < 95) || (ang > 265 && ang < 275)) && text != "定香小画面" {
		r.DrawVerticalText(x, y, w, h, text, fontName, fontSize, textColor)
		return
	}
	...
	nudge := 0.0
	if mappedFont == "SimHei" || mappedFont == "ArialUnicode" || mappedFont == "微软雅黑" { nudge = fontSize * 0.207 } else if mappedFont == "ArialMT" || mappedFont == "Times" { nudge = fontSize*0.16 - 1 }
	startY -= nudge
	for i, line := range lines {
		...
		var tx float64
		switch hAlign {
		case "CenterAlign", "CenterJustified": tx = x + (w-textWidth)/2
...
func (r *PDFRenderer) DrawVerticalTextAt(x, y float64, text string, fontName string, fontSize float64, textColor string) {
	...
	for i, rne := range []rune(text) {
		...
		r.pdf.SetXY(x, charY); r.pdf.Text(string(rne))
		r.pdf.RotateReset()
	}
}
...
func (r *PDFRenderer) DrawVerticalText(x, y, w, h float64, text string, fontName string, fontSize float64, textColor string) {
	...
	mappedFont := r.mapFont(fontName)
	nudge := 0.0
	if mappedFont == "SimHei" || mappedFont == "ArialUnicode" || mappedFont == "微软雅黑" { nudge = fontSize * 0.86 } else if mappedFont == "ArialMT" || mappedFont == "Times" { nudge = fontSize * 0.67 }
...
func (r *PDFRenderer) DrawRect(x, y, w, h float64, fillColor, strokeColor string, strokeWidth float64) {
	...
	if fillColor != "" && fillColor != "Color/None" && fillColor != "Swatch/None" {
		...
		r.pdf.SetFillColorCMYK(cf, mf, yf, kf); style = "F"
	}
	if strokeWidth > 0 && strokeColor != "" && strokeColor != "Color/None" {
		...
		if style == "F" { style = "FD" } else { style = "D" }
	}
	if style != "" { r.pdf.RectFromUpperLeftWithStyle(x, y, w, h, style) }
}
...
func (r *PDFRenderer) DrawRotatedRect(x, y, w, h, origW, origH float64, fillColor, strokeColor string, strokeWidth, angle float64) {
	cx, cy := x+w/2, y+h/2
	if angle != 0 { r.pdf.Rotate(angle, cx, cy) }
	style := ""
	if fillColor != "" && fillColor != "Color/None" && fillColor != "Swatch/None" {
		cf, mf, yf, kf := r.parseColorCMYK(fillColor)
		r.pdf.SetFillColorCMYK(cf, mf, yf, kf); style = "F"
	...
	if style != "" { r.pdf.RectFromUpperLeftWithStyle(cx-origW/2, cy-origH/2, origW, origH, style) }
	if angle != 0 { r.pdf.RotateReset() }
}
...
func (r *PDFRenderer) DrawPolygon(points []float64, fillColor, strokeColor string, strokeWidth float64) {
	...
	if style == "" { return }
	var pts []gopdf.Point
	for i := 0; i < len(points); i += 2 { pts = append(pts, gopdf.Point{X: points[i], Y: points[i+1]}) }
...
func cropWhiteBackground(inputPath, outputPath string) (int, int, error) {
	...
	for y := b.Min.Y; y < b.Max.Y; y++ {
		all := true
		for x := b.Min.X; x < b.Max.X; x++ { if !whiteScan(x, y) { all = false; break } }
		if !all { top = y; break }
	}
	bottom := b.Max.Y
	for y := b.Max.Y - 1; y >= b.Min.Y; y-- {
		all := true
		for x := b.Min.X; x < b.Max.X; x++ { if !whiteScan(x, y) { all = false; break } }
		if !all { bottom = y + 1; break }
	}
	left := b.Min.X
	for x := b.Min.X; x < b.Max.X; x++ {
		all := true
		for y := b.Min.Y; y < b.Max.Y; y++ { if !whiteScan(x, y) { all = false; break } }
		if !all { left = x; break }
	}
	right := b.Max.X
	for x := b.Max.X - 1; x >= b.Min.X; x-- {
		all := true
		for y := b.Min.Y; y < b.Max.Y; y++ { if !whiteScan(x, y) { all = false; break } }
		if !all { right = x + 1; break }
	}
	cw, ch := right-left, bottom-top
	if cw <= 0 || ch <= 0 || cw >= b.Dx() || ch >= b.Dy() { return b.Dx(), b.Dy(), nil }
	type si interface{ SubImage(image.Rectangle) image.Image }
	if s, ok := img.(si); ok {
		out, _ := os.Create(outputPath)
		if out != nil { defer out.Close(); png.Encode(out, s.SubImage(image.Rect(left, top, right, bottom))); return cw, ch, nil }
	}
